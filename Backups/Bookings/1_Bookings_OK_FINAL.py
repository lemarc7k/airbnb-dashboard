...
st.title("📅 Calendario Estilo Airbnb de Reservas")
...
# COLUMNAS
BOOKINGS_COLUMNS = ["Fecha", "Propiedad", "Huésped", "Check-in", "Check-out", "Notas"]
...
for i, row in df_filtrado.iterrows():
    if pd.notnull(row["Check-in"]) and pd.notnull(row["Check-out"]):
        eventos.append({
            "id": str(i),
            "title": f"{row['Huésped']} - {row['Propiedad']}",
...
with st.expander("✏️ Añadir o Editar Reserva"):
    ...
    if st.session_state.edit_id is not None and st.session_state.edit_id < len(df):
        ...
        huesped = st.text_input("👤 Huésped", value=row["Huésped"])
        check_in = st.date_input("📅 Check-in", value=pd.to_datetime(row["Check-in"]).date())
        check_out = st.date_input("📅 Check-out", value=pd.to_datetime(row["Check-out"]).date())
        notas = st.text_area("🗒️ Notas internas", value=row.get("Notas", ""))
    else:
        ...
        notas = st.text_area("🗒️ Notas internas")
        ...
        if st.session_state.edit_id is not None and st.session_state.edit_id < len(df):
            if st.button("✅ Guardar cambios"):
                if not propiedad or not huesped:
                    ...
                    df.loc[st.session_state.edit_id] = [
                        datetime.date.today(), propiedad, huesped, check_in, check_out, notas
                    ]
            ...
            if st.button("➕ Añadir reserva"):
                if not propiedad or not huesped:
                    ...
                    nueva = pd.DataFrame([[datetime.date.today(), propiedad, huesped, check_in, check_out, notas]], columns=BOOKINGS_COLUMNS)
...
st.table(proximos[["Propiedad", "Huésped", "Check-in", "Check-out"]].sort_values("Check-in"))

# ---------- LISTADO VISUAL DE RESERVAS ----------
st.subheader("🗂️ Todas las Reservas (gestión rápida)")
for _, row in df.iterrows():
    row_index = row.name
    with st.expander(f"🏡 {row['Propiedad']} - 👤 {row['Huésped']}"):
        col1, col2, col3 = st.columns([3, 3, 1])
        with col1:
            st.markdown(f"**🛎️ Check-in:** {str(row['Check-in'])[:10]}")
